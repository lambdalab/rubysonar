...
//        clearDiskCache();
    ...
    public String getCachePath(@NotNull String sourcePath) {
        return getCachePath($.getSHA(sourcePath), sourcePath);
    }


    @NotNull
    public String getCachePath(String md5, String name) {
        return $.makePathString(Analyzer.self.cacheDir, name + md5 + ".ast");
    }


    // package-private for testing
    void serialize(@NotNull Node ast) {
        String path = getCachePath($.getSHA(ast.file), new File(ast.file).getName());
